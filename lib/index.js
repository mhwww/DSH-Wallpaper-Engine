...
async function curlDownload(url, dest) {
	...
	for (const proxy of candidates) {
		...
		const args = ["-sL", "--max-time", "40"];
		if (proxy !== null) args.push("-x", proxy);
		args.push("-o", tmp, url);
		try {
			await execFileAsync("curl", args, { timeout: 45_000, windowsHide: true });
...
async function steamPreviewUrl(id) {
	...
	} catch { /* 直连失败时走 curl 代理链解析 JSON */ }
	try {
		const { stdout } = await execFileAsync(
			"curl",
			["-sL", "--max-time", "20", "-d", body, url],
