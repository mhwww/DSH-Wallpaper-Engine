...
/** scene.pkg 解包产物的缓存位置。 */
const pkgCacheDir = () => join(homedir(), ".dsh", "bg-image", "pkg");

/** 在 pkg 解包缓存目录里找某个 id 的背景图（任意扩展名）。 */
async function pkgCacheFile(id) {
	const dir = pkgCacheDir();
	let entries;
	try {
		entries = await readdir(dir);
	} catch {
...
/** 查找 RePKG 工具：环境变量 → ~/.dsh/bin → PATH（由 spawn 隐式尝试）。 */
async function findRepkg() {
	const candidates = [process.env.REPKG_BIN, join(homedir(), ".dsh", "bin", "RePKG.exe")];
	for (const cand of candidates) {
		if (cand && await fileExists(cand)) return cand;
	}
	return "RePKG.exe";
}

/** 防止同一壁纸的解包并发重入。 */
const sceneUpgradeBusy = new Set();

/**
 * 用 RePKG 解包 scene.pkg，取像素量最大的图片作为该壁纸的高清背景。
 * 结果按 id 缓存；工具缺失、无 pkg 或最大图过小（<1600×900）都返回 null，
 * 由调用方回退到 CDN 原图 / 本地预览。
 */
async function extractSceneBackground(id, folder) {
	const cached = await pkgCacheFile(id);
	if (cached !== null) return cached;
	const pkg = join(folder, "scene.pkg");
	if (!(await fileExists(pkg))) return null;
	if (sceneUpgradeBusy.has(id)) return null;
	sceneUpgradeBusy.add(id);
	try {
		const bin = await findRepkg();
		const tmp = join(pkgCacheDir(), `.${id}.extract`);
		await rm(tmp, { recursive: true, force: true });
		await execFileAsync(
			bin,
			["extract", pkg, "-o", tmp, "-e", "tex,png,jpg,jpeg"],
		...
		while (stack.length > 0) {
			...
			for (const entry of await readdir(dir, { withFileTypes: true })) {
				...
				const dim = await sniffImageDim(full);
		...
		if (best === null || bestPx < 1600 * 900) {
			await rm(tmp, { recursive: true, force: true });
			return null;
		}
		const ext = best.toLowerCase().endsWith(".png") ? "png" : "jpg";
		const dest = join(pkgCacheDir(), `${id}.${ext}`);
		await rename(best, dest);
		await rm(tmp, { recursive: true, force: true });
		return dest;
	} catch (error) {
		console.error(`[dsh-bg-image] scene.pkg extract failed for ${id}:`, error instanceof Error ? error.message : error);
		return null;
	} finally {
		sceneUpgradeBusy.delete(id);
	}
}

/** 对外请求的代理候选：常见本地代理端口 → 环境变量 → 直连。
 * 被墙直连要等满超时才失败，所以放最后；端口不存在时连接拒绝是毫秒级的，放前面零成本。 */
...
async function steamPreviewUrl(id) {
	...
	// curlRun 的候选序列已含本地代理优先与直连兜底，无需再走原生 fetch 直连
...
async function resolveWallpaperFile(dirs, id, kind) {
	...
	for (const { dir } of dirs) {
		...
		if (type === "scene" || type === "web") {
			...
			if (full !== null) {
				try {
					// 优先 scene.pkg 内的原生素材（通常 1080p+/4K），其次 Steam CDN 原图
					const pkgBg = await extractSceneBackground(id, folder);
					if (pkgBg !== null) return pkgBg;
...
async function resolveInstantWallpaperFile(dirs, id) {
	if (!/^\d+$/.test(id)) return null;
	const pkgCached = await pkgCacheFile(id);
	if (pkgCached !== null) return pkgCached;
...
function apply(ctx) {
	...
	ctx.inject(["webServer"], (httpCtx) => {
		...
		const serveWallpaperApply = async (req, res) => {
			...
			let upgrading = false;
			if (type === "video") {
				upgrading = !(await fileExists(join(homedir(), ".dsh", "bg-image", "frames", `${id}.jpg`)));
			} else if (type === "scene" || type === "web") {
				upgrading = (await pkgCacheFile(id)) === null && (await steamCacheFile(id)) === null;
			}
			sendJson(res, 200, { value: readConfig(ctx), upgrading });
			if (upgrading) {
				upgradeInBackground(ctx, dirs, id, instant, ownerScope)
					.catch((error) => console.error("[dsh-bg-image] background upgrade failed:", error));
			}
		};
