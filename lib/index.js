	...
	ctx.inject(["webServer"], (httpCtx) => {
		...
		const serveConfig = async (req, res) => {
			...
			const clean = {
				...
				imagePath: typeof section.imagePath === "string" ? normalizePath(section.imagePath) : "",
				opacity: Number(section.opacity),
				sidebarTransparent: section.sidebarTransparent !== false,
				workshopPath: typeof section.workshopPath === "string" ? normalizePath(section.workshopPath) : "",
				hiddenIds: sanitizeHiddenIds(section.hiddenIds, readConfig(ctx).hiddenIds)
			};
			if (!Number.isFinite(clean.opacity)) clean.opacity = 0.35;
			clean.opacity = Math.min(0.9, Math.max(0, clean.opacity));
			try {
		...
		const serveUpload = async (req, res) => {
			...
			try {
				await ownerScope.replace({
					...
					hiddenIds: sanitizeHiddenIds(current.hiddenIds)
				});
