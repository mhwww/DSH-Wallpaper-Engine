...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// 侧栏透明后默认 1px l1 边框在背景图上看不见，背景启用时加强分隔线（颜色随明暗主题）
			'body[data-bg-image-on="1"] .pI_x6G_sidebarCol{border-right:2px solid color-mix(in srgb,var(--dsw-alias-label-primary) 62%,transparent) !important}',
		...
		function buildTokens(cfg, nonce) {
			...
			const image = `url("${IMAGE_URL}?r=${nonce}") center/cover no-repeat fixed`;
			const tokens = {
				"--dsw-alias-bg-base": {
		...
		function apply(ctx) {
			...
			const applyBackground = () => {
				...
				if (cfg !== null && cfg.enabled === true) {
					document.body.dataset.bgImageOn = "1";
				} else {
					delete document.body.dataset.bgImageOn;
				}
			...
			// rc.2 起 settings.plugin.item 是列表槽：官方约定为生成器注册 + key 字段
