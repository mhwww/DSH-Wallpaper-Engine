...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = { enabled: false, imagePath: "", opacity: 0.35, sidebarTransparent: true, workshopPath: "", hiddenIds: [] };
		...
		function Card({ host }) {
			...
			const wallpapersList = allWallpapers.filter((w) => !w.hidden);
				...
				children: [
					...
					jsxs("div", { className: "dbg-wpSection", children: [
						...
						wallpapers !== null && wallpapers.detected && jsxs("div", { className: "dbg-hintInline", children: [
							jsx("span", { children: (wallpapers.auto ? "已自动绑定：" : "已绑定：") + wallpapers.workshopPath + "（" + wallpapers.count + " 张）" })
						] }),
						...
						wallpapersList.length > 0 && jsx("div", {
							...
							children: wallpapersList.map((w) => jsxs("button", {
								...
								title: w.title + "（" + w.type + (w.hd ? "，应用时抽原生分辨率帧" : "") + "）",
								...
								children: [
									...
									w.hd && jsx("span", { className: "dbg-wpBadge", children: "高清帧" }),
									jsx("span", {
